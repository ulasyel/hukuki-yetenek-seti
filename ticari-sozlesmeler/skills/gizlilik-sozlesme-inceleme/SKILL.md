---
name: gizlilik-sozlesme-inceleme
description: >
  ...
  before involving legal. Loaded by /ticari-sozlesmeler:inceleme when an NDA or
...
Check `## Matter workspaces` in `~/.claude/plugins/config/hukuki-yetenek-seti/ticari-sozlesmeler/CLAUDE.md`. If enabled and there is no active matter, ask:

> Bu inceleme hangi dosya için? `/ticari-sozlesmeler:dosya-alani switch <slug>` komutunu çalıştırabilir veya `practice-level` diyebilirsiniz.
...
`~/.claude/plugins/config/hukuki-yetenek-seti/ticari-sozlesmeler/CLAUDE.md` -> `## Playbook` -> matching side -> `NDA triage positions`.
...
- `liquidated damages / penalty` -> `cezai şart`; flag TBK m.179-182 proportionality and reduction risk.
...
- cezai şart and TBK m.179-182 proportionality/reduction risk,
