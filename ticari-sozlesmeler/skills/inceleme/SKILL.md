...
Gelen ticari sözleşmeyi `~/.claude/plugins/config/hukuki-yetenek-seti/ticari-sozlesmeler/CLAUDE.md` içindeki playbook'a göre inceler. Belge türünü tespit eder, uygun alt skill'i çalıştırır ve sonucu tek raporda toplar.
...
1. **Load config.** Read `~/.claude/plugins/config/hukuki-yetenek-seti/ticari-sozlesmeler/CLAUDE.md` and the shared `company-profile.md`. If the commercial profile is missing or still contains placeholders, stop:

   > Önce `/ticari-sozlesmeler:kurulum-mulakati` komutunu çalıştırın. İnceleme yapabilmem için playbook, risk tercihi, yetkili kişiler ve çıktı formatı gerekir.
