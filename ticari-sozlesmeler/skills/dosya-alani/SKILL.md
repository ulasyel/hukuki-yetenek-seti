...
- `/ticari-sozlesmeler:dosya-alani new <slug>` - create a matter workspace.
- `/ticari-sozlesmeler:dosya-alani list` - list active and archived matters.
- `/ticari-sozlesmeler:dosya-alani switch <slug>` - set active matter.
- `/ticari-sozlesmeler:dosya-alani close <slug>` - archive, never delete.
- `/ticari-sozlesmeler:dosya-alani none` - detach to practice-level context.

## Instructions

1. Read `~/.claude/plugins/config/hukuki-yetenek-seti/ticari-sozlesmeler/CLAUDE.md` and confirm `## Matter workspaces`.
2. If disabled, explain that in-house users normally work at practice-level:

   > Dosya çalışma alanları kapalı. Kurum içi hukuk müşavirliği olarak yapılandırıldığınız için eklenti genel pratik bağlamında çalışır. Birden fazla müvekkil veya şirketle çalışıyorsanız `/ticari-sozlesmeler:kurulum-mulakati --redo` ile özel/pratik ayarını güncelleyin.
...
```
~/.claude/plugins/config/hukuki-yetenek-seti/ticari-sozlesmeler/
├── CLAUDE.md
└── matters/
    ├── <slug>/
    │   ├── matter.md
