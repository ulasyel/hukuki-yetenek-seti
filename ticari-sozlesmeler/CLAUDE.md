...
User-specific configuration for this plugin lives at:
  ...
  ~/.claude/plugins/config/hukuki-yetenek-seti/ticari-sozlesmeler/CLAUDE.md

Rules:
1. Skills read configuration from that path, not from this template.
2. If the user config is missing or still contains [PLACEHOLDER], stop before substantive work and ask the user to run /ticari-sozlesmeler:kurulum-mulakati.
...
Bu dosya ticari sözleşme pratiği profilinin şablonudur. İlk kurulumda `/ticari-sozlesmeler:kurulum-mulakati` bu yapıyı kullanarak kullanıcıya ait profili yazar.
...
| Liquidated damages / penalty | Cezai şart (TBK m.179-182) | Check proportionality, one-sidedness, cumulative remedies, cap interaction, and whether a fahiş cezai şart reduction risk should be escalated. |
...
Re-check: `/ticari-sozlesmeler:kurulum-mulakati --check-integrations`
...
**Matters path:** `~/.claude/plugins/config/hukuki-yetenek-seti/ticari-sozlesmeler/matters/`
...
- sınırsız sorumluluk,
- fahiş veya tek taraflı cezai şart (TBK m.179-182),
- FSEK/IP assignment,
...
**Register path:** `~/.claude/plugins/config/hukuki-yetenek-seti/ticari-sozlesmeler/renewal-register.yaml`
