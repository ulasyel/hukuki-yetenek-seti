...
Runs an open source license compliance check against the practice profile in `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`. Classifies dependencies by license family, maps obligations to the deployment model, flags license-unknown and non-OSI-posing-as-OSS packages, and recommends actions — comply, replace, remove, seek legal review, seek commercial license.

## Instructions

1. **Load `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`.** If placeholders present, stop and prompt: "Run `/fikri-mulkiyet:kurulum-mulakati` first — I need to learn your practice profile (and OSS policy, if any) before I can review." If the practice profile points at an uploaded OSS policy, read that too — it is the source of truth for accepted / review / banned licenses on this team.
...
/fikri-mulkiyet:acik-kaynak-inceleme ~/code/my-project/requirements.txt
/fikri-mulkiyet:acik-kaynak-inceleme redis
/fikri-mulkiyet:acik-kaynak-inceleme ~/code/my-project  # repo root — scan all manifests
```
...
**Before scanning dependencies, read `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`.** If it is missing or still contains placeholders, stop and run `/fikri-mulkiyet:kurulum-mulakati`. The practice profile tells you:
...
Prepend the work-product header from `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md` → `## Outputs` (differs by user role — see `## Who's using this`).
