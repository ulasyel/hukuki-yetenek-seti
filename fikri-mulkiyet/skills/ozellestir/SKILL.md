...
The user typed `/fikri-mulkiyet:ozellestir`. They want to change something in their
...
1. **Read the config.** Read
   `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`
   (and `~/.claude/plugins/config/hukuki-yetenek-seti/company-profile.md` one
   level up). If the plugin config does not exist or still contains
   `[PLACEHOLDER]` values, say:

   > You haven't run setup yet. Run `/fikri-mulkiyet:kurulum-mulakati` first —
...
6. **Close.**
   ...
   > run `/fikri-mulkiyet:ozellestir` anytime.
