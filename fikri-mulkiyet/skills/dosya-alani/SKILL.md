...
- `/fikri-mulkiyet:dosya-alani new <slug>` — create a new matter workspace, run a short intake, write `matter.md`
- `/fikri-mulkiyet:dosya-alani list` — list matters with status and active flag
- `/fikri-mulkiyet:dosya-alani switch <slug>` — set the active matter
- `/fikri-mulkiyet:dosya-alani close <slug>` — archive a matter (move to `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/matters/_archived/`, never delete)
- `/fikri-mulkiyet:dosya-alani none` — detach from any active matter, work at practice-level only

## Instructions

1. Read `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md` — confirm the `## Matter workspaces` section is populated. If `Enabled` is `✗`, tell the user: "Matter workspaces are off — you're configured as an in-house practice with one client, so the plugin works from practice-level context automatically. If you actually work across multiple clients, re-run `/fikri-mulkiyet:kurulum-mulakati --redo` and select a private-practice setting. Otherwise, you don't need `/fikri-mulkiyet:dosya-alani` at all." Don't error — the disabled state is the expected one for in-house users.
2. Follow the subcommand logic below.
3. Dispatch on the first token of `$ARGUMENTS`:
   - `new` → run the intake interview, write `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/matters/<slug>/matter.md`, seed `history.md` and `notes.md`.
   - `list` → enumerate `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/matters/*/matter.md`, print a table, mark the active matter.
   - `switch` → update the `Active matter:` line in the practice-level CLAUDE.md.
   - `close` → move `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/matters/<slug>/` to `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/matters/_archived/<slug>/`, log the close date in `history.md`.
...
**Default state is off.** In-house users never see this — they run at practice-level only. Matter workspaces turn on at cold-start for private-practice users, or by editing `## Matter workspaces` in the practice-level CLAUDE.md. If `Enabled` is `✗`, this skill does not run; instead it explains the disabled state and suggests `/fikri-mulkiyet:kurulum-mulakati --redo` for users who actually need matter isolation.
...
```
~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/
...
2. Run the intake interview:
   ...
   - **Matter type** (read the plugin's practice profile for typical categories; for fikri-mulkiyet: marka araştırması (clearance) | marka ihlali (enforcement) | Uyar-Kaldır / 5651 | patent FTO | patent ihlali | IP clause review | OSS compliance | portföy yönetimi | other)
...
6. Do **not** auto-switch to the new matter. Ask: "Want to switch to `<slug>` now? (`/fikri-mulkiyet:dosya-alani switch <slug>`)"
...
1. Confirm `matters/<slug>/matter.md` exists. If not, offer `/fikri-mulkiyet:dosya-alani new <slug>`.
