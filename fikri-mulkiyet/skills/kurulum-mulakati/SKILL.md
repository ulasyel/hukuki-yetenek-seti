...
Runs the cold-start interview. First run writes `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`; subsequent runs with `--redo` re-interview and show a diff before overwriting.

## Instructions

1. **Check current state:** Read `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`. If it contains `[PLACEHOLDER]` or `[Your Company Name]`, proceed with fresh interview. If populated and `--redo` not passed, ask: "Looks like you're already set up. Want to re-run the interview? This will overwrite `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md` (I'll show you a diff first)."
...
5. **Migration:** If a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `~/.claude/plugins/cache/hukuki-yetenek-seti/fikri-mulkiyet/*/CLAUDE.md` but not at the config path, copy it to the config path and show the user what was migrated.

6. **Write `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`** (create parent directories as needed) per the structure below. Use the lawyer's own words where possible.

7. **Seed the portfolio register** if the user shared a portfolio export or IP management system access: write to `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/portfolio.yaml`. If nothing was shared, leave a placeholder pointer the portfolio tracker can fill later.

8. **Show summary + propose next steps:**
   - "Here's what I heard — `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md` is written. What did I get wrong?"
...
Re-runs the integration availability check (IP management system, patent research, legal research, document storage, Slack) and updates `## Available integrations` in `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`. Does not re-interview. Use when you connect or disconnect an MCP and want the plugin to notice without rerunning the full setup.
...
```
/fikri-mulkiyet:kurulum-mulakati
```

```
/fikri-mulkiyet:kurulum-mulakati --redo
```

```
/fikri-mulkiyet:kurulum-mulakati --check-integrations
```
...
Read `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`:
...
If a CLAUDE.md exists at the old cache path `~/.claude/plugins/cache/hukuki-yetenek-seti/fikri-mulkiyet/*/CLAUDE.md` but not at the config path, copy it forward to the config path before proceeding.
...
> **`fikri-mulkiyet` is for people who manage trademarks, copyrights, patents, trade secrets, and open source obligations — clearance, enforcement, portfolio tracking, and IP clauses in agreements.**
>
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/fikri-mulkiyet:kurulum-mulakati` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md` with a `<!-- SETUP PAUSED AT: [section name] — run /fikri-mulkiyet:kurulum-mulakati to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
You don't need all of these. Core features work with file access alone. If you set something up later, re-run `/fikri-mulkiyet:kurulum-mulakati --check-integrations`.
...
Write to `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md` (create parent directories as needed). If the user shared a portfolio export, also seed `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/portfolio.yaml` with the extracted registrations.
...
>
> - **Clear a proposed trademark** — e.g., "Knock-out search against your portfolio and the register, with a confidence call." Try: `/fikri-mulkiyet:benzerlik-taramasi`
> - **Triage a potential infringement** — e.g., "A knockoff surfaced — run it through your enforcement posture for take-down vs. cease-and-desist vs. monitor." Try: `/fikri-mulkiyet:tecavuz-triyaji`
> - **Freedom-to-operate analysis** — e.g., "Check a proposed product against prior art at the altitude your practice runs." Try: `/fikri-mulkiyet:engel-taramasi`
> - **Draft a takedown or cease-and-desist** — e.g., "From intake to drafted letter in house voice, with escalation routing." Try: `/fikri-mulkiyet:ip-ihtarnamesi`
> - **Open-source compliance check** — e.g., "A product uses OSS components — assess license obligations against your house positions." Try: `/fikri-mulkiyet:acik-kaynak-inceleme`
> - **Portfolio renewal status** — e.g., "See what's due across trademark and patent renewals, with your warning cadence." Try: `/fikri-mulkiyet:portfoy-takip`
>
...
4. **Close with a note on changeability.** End with something like:
   ...
   > "Done. Your practice profile is at `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md` — it's a plain text file you can read and edit directly. Anything you answered can be changed:
   >
   > - Edit the file directly for a quick change (a new approver, a revised watch list, a jurisdiction swap)
   > - Run `/fikri-mulkiyet:kurulum-mulakati --redo` for a full re-interview
   > - Run `/fikri-mulkiyet:kurulum-mulakati --check-integrations` to re-check what's connected
   >
