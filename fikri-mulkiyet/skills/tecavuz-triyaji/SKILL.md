...
1. Read `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`. If it
   contains `[PLACEHOLDER]`, stop and direct to `/fikri-mulkiyet:kurulum-mulakati`.
...
8. End with recommended next steps, the non-lawyer gate if the role is
   ...
   draft the C&D via `/fikri-mulkiyet:ip-ihtarnamesi` or the takedown via
   `/fikri-mulkiyet:uyar-kaldirir`. Do not draft automatically.
...
```
/fikri-mulkiyet:tecavuz-triyaji "competitor launched a tool called APEXSEED in class 9 — we have APEXLEAF registered in class 9; likely confusion?"
```

```
/fikri-mulkiyet:tecavuz-triyaji "former engineer took notes on our model architecture to a competitor — possible trade secret?"
```

```
/fikri-mulkiyet:tecavuz-triyaji
```
...
**Matter context.** Check `## Matter workspaces` in the practice-level CLAUDE.md. If `Enabled` is `✗` (the default for in-house users), skip the rest of this paragraph — skills use practice-level context and the matter machinery is invisible. If enabled and there is no active matter, ask: "Which matter is this for? Run `/fikri-mulkiyet:dosya-alani switch <slug>` or say `practice-level`." Load the active matter's `matter.md` for matter-specific context and overrides. Write outputs to the matter folder at `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/matters/<matter-slug>/`. Never read another matter's files unless `Cross-matter context` is `on`.
...
Read `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`. Pull:
...
- **Integrations** from `## Available integrations` — Yargı PRO,
...
>
> **Two choices:**
> - Run `/fikri-mulkiyet:kurulum-mulakati` (2 minutes) to configure your profile, then I'll run this tailored to YOUR practice.
...
> "That was a generic run against default assumptions. Run `/fikri-mulkiyet:kurulum-mulakati` to get output calibrated to YOUR practice — your playbook, your jurisdiction, your risk appetite. 2 minutes."
...
**Route to `/fikri-mulkiyet:engel-taramasi` for the detailed framework.** This mode is the
...
- **Tasarım tescilleri** (Endüstriyel Tasarımların Korunması Hakkında KHK kapsamında) farklı test, farklı
  claim yapısı ve farklı tazminat rejimi taşır. Tasarım tesciline unsurlu tablo
  kurma, eşdeğerlik doktrini veya eleman-eleman eşleme UYGULANMAZ; tasarım
  tescili çizimlerle tanımlanan tek tescildir.
- **`RE` öneki — yeniden verilen patent.** Devraldığı faydalı model/patent gibi işlem görür;
  ancak yeniden verme-özel def'ileri (ara haklar, geri alma kuralı) bayraklanır.
- **Bitki çeşitleri** ayrı kanunla korunur (5042 sayılı Tohumculuk Kanunu). Vejetatif olarak
...
Prepend the work-product header from `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md` `## Outputs`.
...
- [routing per `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`
...
`~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/matters/<matter-slug>/outputs/infringe-<mode>-<subject-slug>-YYYY-MM-DD.md`.
Otherwise write to
`~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/outputs/infringe-<mode>-<subject-slug>-YYYY-MM-DD.md`
...
> Want me to draft a cease-and-desist on this? Run `/fikri-mulkiyet:ip-ihtarnamesi`.
...
> Want me to prepare an Uyar-Kaldır (Takedown) notice? Run `/fikri-mulkiyet:uyar-kaldirir`.
...
- **Draft the C&D, takedown, or complaint.** Those are separate skills
  (`/fikri-mulkiyet:ip-ihtarnamesi`, `/fikri-mulkiyet:uyar-kaldirir`) gated by the approval
