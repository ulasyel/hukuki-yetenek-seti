...
```
/fikri-mulkiyet:engel-taramasi "an on-device speech recognition model for consumer wearables, US launch first"
```

```
/fikri-mulkiyet:engel-taramasi
```
...
> product is clear. Patent tecavüzü kusura dayanmaz; bilinçli tecavüz
> (patenti bilerek devam etme) 6769 sayılı SMK'da artırılmış tazminat ve
> men'i sonuçlarına yol açabilir. Lansman, üretim, kullanım, satış veya ithalat kararı bir iş kararidir ve resmi FTO çalışmasıyla desteklenmelidir.
...
Before running triage, read `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`. Pull:
...
If `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md` contains `[PLACEHOLDER]` or `[Your Company Name]`, surface this bounce:
...
>    imported? (Each can be a separate infringing act under SMK patent provisions. I'll
>    default to Türkiye if you don't specify.)
...
Prepend the work-product header from `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md` `## Outputs`. Mark the document as privileged if the role is lawyer; see the non-lawyer gate below if not.
...
`~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/matters/<matter-slug>/outputs/fto-triage-<subject-slug>-YYYY-MM-DD.md`.
Otherwise write to
`~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/outputs/fto-triage-<subject-slug>-YYYY-MM-DD.md`
...
- **Handle trade-secret or trademark analysis** — use `/fikri-mulkiyet:tecavuz-triyaji`
  with the right mode.
