...
1. Read `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`. If it
   contains `[PLACEHOLDER]`, stop and direct to `/fikri-mulkiyet:kurulum-mulakati`. If the
...
```
/fikri-mulkiyet:bulus-girdisi "a new cache-eviction algorithm that uses a learned model rather than LRU; conceived Q1 this year, not yet disclosed, engineering prototype in internal staging"
```

```
/fikri-mulkiyet:bulus-girdisi
```
...
this for? Run `/fikri-mulkiyet:dosya-alani switch <slug>` or say `practice-level`." Load
the active matter's `matter.md` for matter-specific context and overrides.
Write outputs to the matter folder at
`~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/matters/<matter-slug>/`.
...
`~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`.** If it is
missing or still contains placeholders, stop and run `/fikri-mulkiyet:kurulum-mulakati`. The
