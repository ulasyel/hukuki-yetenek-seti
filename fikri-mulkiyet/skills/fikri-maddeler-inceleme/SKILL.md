...
# /fikri-maddeler-inceleme

Reviews the IP clauses in an agreement against the practice profile in `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`. Flags assignment gaps, ownership ambiguity, license-scope issues, and IP warranty/indemnity problems. Produces a memo with per-clause findings, prioritized by risk, with suggested redline language where appropriate.

## Instructions

1. **Load `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`.** If placeholders present, stop and prompt: "Run `/fikri-mulkiyet:kurulum-mulakati` first — I need to learn your practice profile before I can review IP clauses against it."
...
```
/fikri-mulkiyet:fikri-maddeler-inceleme ~/Documents/vendor-sow.pdf
/fikri-mulkiyet:fikri-maddeler-inceleme https://docs.google.com/document/d/...
/fikri-mulkiyet:fikri-maddeler-inceleme
```
...
**Before reading the agreement, read `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`.** If it is missing or still contains placeholders, stop and run `/fikri-mulkiyet:kurulum-mulakati`. The practice profile tells you:
...
**Escalation:** Per `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md`, assignment-scope gaps escalate to [approver].
```
...
Prepend the work-product header from `~/.claude/plugins/config/hukuki-yetenek-seti/fikri-mulkiyet/CLAUDE.md` → `## Outputs` (it differs by user role — see `## Who's using this`).
