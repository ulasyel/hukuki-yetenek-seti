...
- `/sirketler-hukuku:dosya-alani new <slug>` — create a new matter workspace, run a short intake, write `matter.md`
- `/sirketler-hukuku:dosya-alani list` — list matters with status and active flag
- `/sirketler-hukuku:dosya-alani switch <slug>` — set the active matter
- `/sirketler-hukuku:dosya-alani close <slug>` — archive a matter (move to `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/matters/_archived/`, never delete)
- `/sirketler-hukuku:dosya-alani none` — detach from any active matter, work at practice-level only

## Instructions

1. Read `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/CLAUDE.md` — confirm the `## Matter workspaces` section is populated. If `Enabled` is `✗`, tell the user: "Matter workspaces are off — you're configured as an in-house practice with one client, so the plugin works from practice-level context automatically. If you actually work across multiple clients, re-run `/sirketler-hukuku:kurulum-mulakati --redo` and select a private-practice setting. Otherwise, you don't need `/matter-workspace` at all." Don't error — the disabled state is the expected one for in-house users.
2. Use the workflow below.
3. Dispatch on the first token of `$ARGUMENTS`:
   - `new` → run the intake interview, write `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/matters/<slug>/matter.md`, seed `history.md` and `notes.md`.
   - `list` → enumerate `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/matters/*/matter.md`, print a table, mark the active matter.
   - `switch` → update the `Active matter:` line in the practice-level CLAUDE.md.
   - `close` → move `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/matters/<slug>/` to `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/matters/_archived/<slug>/`, log the close date in `history.md`.
...
**Default state is off.** In-house users never see this — they run at practice-level only. Matter workspaces turn on at cold-start for private-practice users, or by editing `## Matter workspaces` in the practice-level CLAUDE.md. If `Enabled` is `✗`, this skill does not run; `/sirketler-hukuku:dosya-alani` explains the disabled state and suggests `/sirketler-hukuku:kurulum-mulakati --redo` for users who actually need matter isolation.
...
```
~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/
...
2. Run the intake interview:
   ...
   - **Matter type** (read the plugin's practice profile for typical categories; for sirketler-hukuku: M&A buy-side | M&A sell-side | financing | board matter | entity reorg | integration project | other)
...
6. Do **not** auto-switch to the new matter. Ask: "Want to switch to `<slug>` now? (`/sirketler-hukuku:dosya-alani switch <slug>`)"
...
1. Confirm `matters/<slug>/matter.md` exists. If not, offer `/sirketler-hukuku:dosya-alani new <slug>`.
...
- [e.g., "Uygulanacak hukuk: Türk hukuku olmalı; yabancı hukuk seçimi ayrıca eskale edilecek."]
