...
description: >
  ...
  per the trust level in `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/CLAUDE.md`. Use when user says "send to Luminance",
...
**Matter context.** Check `## Matter workspaces` in the practice-level CLAUDE.md. If `Enabled` is `✗` (the default for in-house users), skip the rest of this paragraph — skills use practice-level context and the matter machinery is invisible. If enabled and there is no active matter, ask: "Which matter is this for? Run `/sirketler-hukuku:dosya-alani switch <slug>` or say `practice-level`." Load the active matter's `matter.md` for matter-specific context and overrides. Write outputs to the matter folder at `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/matters/<matter-slug>/`. Never read another matter's files unless `Cross-matter context` is `on`.
...
**Before you hand off:** try `tabular-review` first (`/sirketler-hukuku:tablo-inceleme`). For anything the user's environment can handle — a few hundred documents, a defined column schema — native tabular review is faster to set up, has no per-document cost, and keeps the work product local. Hand off to Luminance/Kira when the corpus is genuinely too large, the team already has a license and workflow, or the matter requires a tool with a validated provenance chain.

## Load context

`~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/CLAUDE.md` → AI-assisted review:
...
If `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/CLAUDE.md` says no AI tool → this skill is a no-op. Everything goes through diligence-issue-extraction directly.
...
- Specify extraction targets per `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/CLAUDE.md` (which clause types)
- Note the materiality threshold so tool output can be filtered

### Step 2: Load (or instruct the loader)

Per `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/CLAUDE.md` — who loads. If it's you, generate the load instructions. If it's someone else, generate the request:
...
- Exclusivity
- [etc. per `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/CLAUDE.md`]
...
**"Use as-is":** Ingest directly into diligence findings. (Only if `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/CLAUDE.md` says this — it's rare.)
...
**Trust level:** [per `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/CLAUDE.md`]
...
- It doesn't replace the tool's output with its own judgment entirely — if `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/CLAUDE.md` says spot-check 10%, check 10%, not 100%.
- It doesn't decide the trust level — that's in `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/CLAUDE.md`, set at cold-start based on the team's experience with the tool.
