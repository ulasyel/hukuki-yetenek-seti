...
5. `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/deals/[kodu]/integration-tracker.yaml` dosyasını oku/yaz.
...
**Dosya Bağlamı.** Büro düzeyindeki CLAUDE.md dosyasında `## Matter workspaces` bölümünü kontrol et. Eğer `Enabled` durumu `✗` ise (kurum içi hukukçular için varsayılan), bu paragrafı atla. Eğer aktifse ve açık bir dosya (matter) yoksa şunu sor: "Bu hangi dosya için? `/sirketler-hukuku:dosya-alani switch <slug>` komutunu çalıştır veya `buro-geneli` de." Aktif dosyanın `matter.md` içeriğini bağlam ve özel talimatlar için yükle. Çıktıları dosya klasörüne (`~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/matters/<dosya-slug>/`) yaz. `Cross-matter context` açık değilse asla başka bir dosyanın verisini okuma.
...
Konumu: `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/deals/[kodu]/integration-tracker.yaml`. 
...
```
/sirketler-hukuku:entegrasyon-yonetimi --init [--deal [kodu]]
```

### Adım 1: Dosya Bağlamını Yükle

`~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/deals/[kodu]/deal-context.md` oku. Bulunamazsa: işlem kodu, hedef şirket, kapanış tarihi, işlem lideri ve dışarıdan hizmet alınan büro (outside counsel) bilgilerini sor. Yoksa `deal-context.md` oluştur ve yaz.

Varsa `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/deals/[kodu]/closing-checklist.yaml` dosyasını oku. Kapanış sonrası (post-closing) olarak işaretlenen tüm öğeler, 1. Gün veya 30. Gün iş planı öğesi haline gelir (durumları kapanış kontrol listesinden miras alınır).
...
> - Kapanış Kontrol Listesi — eğer M&A başlangıç (cold-start) skill'i tarafından oluşturulduysa, onu `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/deals/[kodu]/closing-checklist.yaml` yolundan otomatik alırım.
...
Sonraki adım: Sözleşme listesini aktarmak için /sirketler-hukuku:entegrasyon-yonetimi --contracts komutunu, ardından ilk durum özetinizi görmek için --report komutunu çalıştırın.
```
...
```
/sirketler-hukuku:entegrasyon-yonetimi --contracts [--deal [kodu]]
```
...
```
/sirketler-hukuku:entegrasyon-yonetimi --report [--deal [kodu]]
```
...
```
/sirketler-hukuku:entegrasyon-yonetimi --update [--deal [kodu]]
```
