...
Aktif due diligence sürecinde her gün çalışır. Kontrol listesi durum raporlaması, `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/CLAUDE.md` → İşlem ekibi bilgilendirme periyodu (Deal team briefing cadence) ayarına göre yapılır.

## Entegrasyonlar

Slack'e mesaj göndermek, ortamınızda bir Slack MCP sunucusunun bulunmasını gerektirir. Bu eklenti doğrudan bir Slack MCP sunucusu içermez. Eğer Slack MCP kurulu değilse, VDR güncellemelerini ve kontrol listesi durumunu `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/deals/[kodu]/updates/[tarih].md` dosyasına yazın ve kullanıcıya bildirin — sessizce hata verip kapanmayın.

Aynı şekilde VDR araçları (Box, Intralinks, Datasite vb.) harici MCP'lerdir — eğer bağlı değillerse, kullanıcıdan VDR dışa aktarım (export) dosyasını yüklemesini isteyin veya `~/.claude/plugins/config/hukuki-yetenek-seti/sirketler-hukuku/deals/[kodu]/vdr-inventory.md` dosyasını manuel olarak güncellemesini talep edin.
