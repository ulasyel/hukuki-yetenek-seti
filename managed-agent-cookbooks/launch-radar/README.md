...
- **Risk calibration.** The `risk-classifier` reads the user's calibration from `../../urun-hukuku/CLAUDE.md` (populated by `/urun-hukuku:kurulum-mulakati`). If you haven't run cold-start, either do that first or hand-author a CLAUDE.md with "Usually blocks / Usually requires work / Usually FYI" tables before the first scan. Without calibration the classifier falls back to keyword triggers only, which is noisy.
