...
Yapılandırmanız `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md` konumunda saklanır ve eklenti güncellemelerinden etkilenmez.

## Komutlar

| Komut | İşlevi |
|---|---|
| `/dava-yonetimi:kurulum-mulakati` | Kurulum → Kurumsal profili yazar: `CLAUDE.md` |
| `/dava-yonetimi:dosya-acilis` | Standart Kayıt → `matters/[kisa_ad]/` yazar ve `_log.yaml`'a ekler (Arabuluculuk şartı, Dava/Harç Değeri vb. dâhil) |
| `/dava-yonetimi:portfoy-durum` | Portföy Özeti — Risk dağılımı, yaklaşan kesin süreler/duruşmalar, hareketsiz dosyalar |
| `/dava-yonetimi:dosya-brifingi [slug]` | Tek dosyaya derin bakış — Başmüşavir veya dış büro toplantısı öncesi okumaya hazır brifing |
| `/dava-yonetimi:dosya-guncelleme [slug]` | Dosya tarihçesine tarihli bir olay (Örn. Tensip, Ara Karar) ekler; logdaki `last_updated` bilgisini yeniler |
| `/dava-yonetimi:dosya-kapatma [slug]` | Dosyayı aktif portföyden çıkararak arşivler (silinmez, elde tutulur) |
| `/dava-yonetimi:ihtarname-girdisi [başlık]` | İhtarname (Ödeme/Fesih/İhlal vb.) öncesi bağlam toplama aşaması |
| `/dava-yonetimi:ihtarname-hazirla [slug]` | İhtarname taslağını oluşturur — Gizlilik filtrelerinden geçer, `.docx` üretir, noter/KEP gönderim kontrol listesi yazar |
| `/dava-yonetimi:gelen-ihtarname-triyaji [yol]` | Gelen ihtarname/icra emri triyajı — Seçenek analizi (Cevap verme/menfi tespit vs.), portföy çapraz kontrolü |
| `/dava-yonetimi:muzekkere-triyaji [yol]` | Kurum/Mahkeme/Savcılık Müzekkeresi (Subpoena) triyajı — Bilgi/belge talebini sınıflandırır (KVKK m.28 dengesi), itiraz çerçevesi ve yanıt planı üretir |
| `/dava-yonetimi:delil-muhafaza [slug] [--issue/--refresh/--release/--status]` | Şirket İçi Kayıt/Delil Muhafaza Talimatı — İlgili birimlere (IT, İK) "silmeyin" ihtarı yazar ve Delil Tespiti (HMK m. 400) hazırlığı yapar |
| `/dava-yonetimi:kronoloji [slug]` | İddia/Savunmanın genişletilmesi yasağına hazırlık — Sunulan belgelerden veya dosyalardan kronoloji çıkarır, her olayı uyuşmazlık teorisine göre etiketler |
| `/dava-yonetimi:dis-buro-durum` | Dış hukuk bürolarından haftalık UYAP / dava safahat raporu isteyen taslaklar (Email MCP varsa direkt Gmail taslağı) |
| `/dava-yonetimi:dava-unsur-tablosu` | Unsur Tablosu (Claim Chart) — Fikri Sınai Haklar (Patent/Marka tecavüzü, hükümsüzlük) veya ticari davalar için boşluk tespiti sağlayan iddia-delil eşleştirme şeması |
