...
5. Append final entry to `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/[slug]/history.md`.
6. Matter stays in `_log.yaml` and `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/[slug]/` — not deleted. `/portfolio-status` filters it from active rollups.
...
- `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/[slug]/matter.md` — reference (intake context)
...
> "I don't see [matter slug] in the matter log. Nothing to close — either the slug is wrong or the matter was never intaken through `/dava-yonetimi:dosya-acilis`. Check the slug first; if it genuinely was never intaken, there's no row to update and no file structure to close."
...
**Before closing the matter (the consequential act — the matter is archived and active tracking ends):** Read `## Who's using this` in `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md`. If the Role is Non-lawyer:
...
### Append final entry to `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/[slug]/history.md`
...
### Touch `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/[slug]/matter.md`
