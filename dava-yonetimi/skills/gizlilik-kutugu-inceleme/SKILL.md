...
# /gizlilik-kutugu-inceleme (Gizlilik ve Sır Saklama İncelemesi)

1. `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md` yükle → inceleme protokolü, log formatı.
...
**Dosya bağlamı.** Kurumsal düzeydeki CLAUDE.md'de `## Dosya Çalışma Alanları`nı kontrol edin. Eğer aktif bir dosya yoksa sorun: "Bu hangi uyuşmazlık için? `/dava-yonetimi:dosya-alani switch <kisa-ad>` çalıştırın." Asla `Cross-matter context` açık değilse başka dosyanın evraklarını okumayın.
