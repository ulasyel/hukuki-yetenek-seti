...
The user typed `/dava-yonetimi:ozellestir`. They want to change something
...
1. **Read the config.** Read
   ...
   > You haven't run setup yet. Run `/dava-yonetimi:kurulum-mulakati`
...
6. **Close.**
   ...
   > run `/dava-yonetimi:ozellestir` anytime.
...
- **Flag internal inconsistency.** If the change would make the profile
  ...
  tension.
- **Flag guardrail degradation.** The ihtarname gates on
  `/demand-draft` (mesleki sır / ikrar riski / arabuluculuk gizliliği /
  TTK m.18/3 bildirim şekli), the privilege header on matter outputs, source
