...
2. Confirm slug exists in `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/` and `_log.yaml`.
...
4. Append dated entry to `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/[slug]/history.md`.
...
- `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/_log.yaml` — find the row
- `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/[slug]/history.md` — append target
- `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/[slug]/matter.md` — reference (don't rewrite)
- `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md` — risk calibration (if re-assessing risk)
...
> "I don't see [matter slug] in the matter log. Run `/dava-yonetimi:dosya-acilis` first so the conflicts check runs and the matter workspace exists. I won't append history to an unmanaged matter — the conflicts check is the gate, and there's no `history.md` to append to until the matter is intaken."
...
If the Strategy update is a **settlement acceptance** (the company is accepting a settlement offer, executing a settlement agreement, or authorizing acceptance in principle — not merely logging an offer made or received): Read `## Who's using this` in `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md`. If the Role is Non-lawyer:
...
If materiality moves to `reserved` or `disclosed`, and the matter did not previously carry a reserve or disclosure, flag the event as requiring finance / audit-committee notification per `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md` materiality thresholds.
...
### Append to `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/[slug]/history.md`
...
### Update `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/_log.yaml`
