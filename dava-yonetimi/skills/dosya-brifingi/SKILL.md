...
1. Load `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md` → risk calibration + relevant stakeholders.
2. Follow the workflow and reference below.
3. Read `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/[slug]/matter.md` + `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/[slug]/history.md` + log row from `_log.yaml`.
...
- `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/_log.yaml` — structured row
- `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/[slug]/matter.md` — narrative intake
- `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/[slug]/history.md` — event log
- `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md` — risk calibration (so "risk: high" means something specific, not generic)
...
> "I don't see [matter slug] in the matter log. Run `/dava-yonetimi:dosya-acilis` first so the conflicts check runs and the matter workspace is set up. I won't build a briefing on a matter that hasn't been intaken — the conflicts check is the gate."
...
- Dosyaya yeni bir paydaş mı katılmalı? (örn. veri ihlali gelişmesi sonrası CISO'nun devreye girmesi gerekir mi)?
...
If `last_updated > 30 days ago`: flag at the top AND suggest running `/dava-yonetimi:dosya-guncelleme [slug]` after the meeting to capture whatever's discussed.
