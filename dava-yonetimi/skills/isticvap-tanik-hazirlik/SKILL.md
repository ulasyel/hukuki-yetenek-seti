...
# /isticvap-tanik-hazirlik (İsticvap / Tanık Hazırlığı)

1. `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md` yükle → dava teorisi, kilit olgular.
...
`~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md` → dava teorisi (teori, kırılma olgusu, lehe/aleyhe kilit olgular).

**Güvenlik Kapısı (Gate).** Taslağı oluşturmadan önce `_log.yaml` dosyasında dosya kısa adını (slug) kontrol et. Yoksa reddet:
> "Dosya günlüğünde [kısa ad] bulamıyorum. Önce `/dava-yonetimi:dosya-acilis` çalıştırın. Kaydedilmemiş bir dosya için tanık taslağı hazırlamam."
