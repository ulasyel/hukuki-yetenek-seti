...
2. Aksi takdirde: `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/[kisa-ad]/matter.md` ve log satırını yükle.
3. `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md` yükle → gizlilik işaretleri, muhafaza şablonu işaretçisi, eskalasyon kuralları.
...
- `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/_log.yaml` — log satırı (legal_hold alanları + durum)
- `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/[kisa-ad]/matter.md` — dosya bağlamı (karşı taraf, olaylar, iç sorumlular)
- `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md` — şablon işaretçisi, eskalasyon normları
...
> "Dosya günlüğünde [kısa ad] bulamıyorum. Önce `/dava-yonetimi:dosya-acilis` çalıştırın ki dosya kurulsun. Kaydedilmemiş (intake yapılmamış) bir dosya için talimat/bildirim yayımlamam."
