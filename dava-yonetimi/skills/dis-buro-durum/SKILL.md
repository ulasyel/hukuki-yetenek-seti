...
# /dis-buro-durum (Dış Hukuk Bürosu Safahat Durumu)

Haftalık çalıştırmak için `/dava-yonetimi:dis-buro-durum` komutunu anımsatıcıya ekleyin. Otomatik zamanlama (scheduling) eklentide yerleşik değildir.

1. `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/_log.yaml` dosyasını yükle, kurallara göre filtrele.
2. `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md` yükle → dış büro iletişim dili, imza varsayılanları, bütçe/avans duruşu.
...
5. Markdown'ı buraya yaz: `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/oc-status/[YYYY-MM-DD]/[slug].md`.
6. Gmail MCP yetkilendirilmişse: Gmail taslakları oluştur. Değilse: Sadece markdown oluştur ve özette belirt.
7. Özet raporunu yaz `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/oc-status/[YYYY-MM-DD]/_summary.md` — ne çalıştı, ne atlandı ve neden.
...
- `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/_log.yaml` — filtreleme ve alan kaynağı
- `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/[slug]/matter.md` — dosya bağlamı (mevcut aşama, açık sorular)
- `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/[slug]/history.md` — ne sorulacağını belirlemek için son olaylar
- `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md` → kurumsal iletişim dili, imza sahibi, bütçe/masraf avansı duruşu
...
Tonu `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md` dış büro iletişim stiline göre ayarlayın — bazı şirketler "Sayın Avukat" şeklinde resmidir, bazıları "Merhaba [İlk İsim]". Uyarlayın.

## Çıktı

### Markdown Taslakları

Yazılacak yer: `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/oc-status/[YYYY-MM-DD]/[slug].md`
...
Tüm dosyalar işlendikten sonra `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/oc-status/[YYYY-MM-DD]/_summary.md` dosyasını yazın:
...
Bu beceri haftalık çalıştırılmak üzere tasarlanmıştır. `/dava-yonetimi:dis-buro-durum` komutunu takviminize bir anımsatıcı (Örn: Pazartesi sabahı) olarak ekleyebilirsiniz.
