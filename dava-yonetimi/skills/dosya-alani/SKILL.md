...
- `/dava-yonetimi:dosya-alani new <slug>` — create a new matter workspace, run a short intake, write `matter.md`
- `/dava-yonetimi:dosya-alani list` — list matters with status and active flag
- `/dava-yonetimi:dosya-alani switch <slug>` — set the active matter
- `/dava-yonetimi:dosya-alani close <slug>` — archive a matter (move to `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/_archived/`, never delete)
- `/dava-yonetimi:dosya-alani none` — detach from any active matter, work at practice-level only

Note: `/dava-yonetimi:dosya-brifingi [slug]` (no subcommand) is a separate command that produces a briefing on a specific matter — useful for in-house portfolio review. Matter workspace management lives here.

## Instructions

1. Read `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md` — confirm the `## Matter workspaces` section is populated. If `Enabled` is `✗`, tell the user: "Matter workspaces are off — you're configured as an in-house practice with one client, so the plugin works from practice-level context automatically. If you actually work across multiple clients, re-run `/dava-yonetimi:kurulum-mulakati --redo` and select a private-practice setting. Otherwise, you don't need `/matter-workspace` at all." Don't error — the disabled state is the expected one for in-house users.
2. Follow the workflow and reference below.
3. Dispatch on the first token of `$ARGUMENTS`:
   - `new` → run the intake interview, write `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/<slug>/matter.md`, seed `history.md` and `notes.md`.
   - `list` → enumerate `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/*/matter.md`, print a table, mark the active matter.
   - `switch` → update the `Active matter:` line in the practice-level CLAUDE.md.
   - `close` → move `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/<slug>/` to `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/_archived/<slug>/`, log the close date in `history.md`.
...
```
~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/
...
2. Run the intake interview:
   ...
   - **Matter type** (read the plugin's practice profile for typical categories; for dava-yonetimi: contract dispute | employment | IP | regulatory / investigation | product liability | class action | other)
...
6. Do **not** auto-switch to the new matter. Ask: "Want to switch to `<slug>` now? (`/dava-yonetimi:dosya-alani switch <slug>`)"
...
1. Confirm `matters/<slug>/matter.md` exists. If not, offer `/dava-yonetimi:dosya-alani new <slug>`.
