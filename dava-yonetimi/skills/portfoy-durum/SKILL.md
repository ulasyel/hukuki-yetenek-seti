...
# /portfoy-durum (Portföy ve Dosya Durumu)

1. `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md` yükle → risk kalibrasyonu (`risk:` alanının nasıl okunacağını tanımlar).
2. Aşağıdaki iş akışını ve referansları izle.
3. `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/_log.yaml` dosyasını ayrıştır. Varsayılan olarak kapalı (closed) dosyaları filtrele (`--all` ile dahil et).
...
- `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/_log.yaml` — tek doğru kaynak (source of truth)
- `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md` — risk kalibrasyonu (risk/önem derecesi alanlarını Türk pratiğine göre doğru yorumlamak için)
