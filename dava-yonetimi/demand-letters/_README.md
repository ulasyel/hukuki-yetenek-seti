...
1. `/dava-yonetimi:ihtarname-girdisi [başlık]` → adaptif bilgi alma sürecini (intake) çalıştırır, `intake.md` dosyasını yazar.
2. `/dava-yonetimi:ihtarname-hazirla [kisa-ad]` → Sulh görüşmeleri (arabuluculuk) / Av.K. m.36 gizlilik kontrol listesini çalıştırır, `.docx` taslağını oluşturur, `checklist.md` yazar ve bir "Dava" (matter) oluşturmayı teklif eder.
