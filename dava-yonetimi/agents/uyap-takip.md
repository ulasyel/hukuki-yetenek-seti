...
Takvim, `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md` → Sık Kullanılan Adliyeler ve `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/matters/_log.yaml` içindeki dosya bazlı periyotlara göre belirlenir.
...
1. `~/.claude/plugins/config/hukuki-yetenek-seti/dava-yonetimi/CLAUDE.md` dosyasını kurum stili, eskalasyon kuralları ve mahkeme usulleri için okur. `_log.yaml` dosyasından aktif portföyü (dosya `id`, `mahkeme/esas no`, son kontrol zamanı ve bekleyen işler) alır.
